...
EVENT_MAPPER = {
	"orders/create": "fateh_shopify_connector.fateh_shopify_connector.order.sync_sales_order",
	"orders/paid": "fateh_shopify_connector.fateh_shopify_connector.order.process_paid_order",
	"orders/cancelled": "fateh_shopify_connector.fateh_shopify_connector.order.cancel_order",
	"orders/fulfilled": "fateh_shopify_connector.fateh_shopify_connector.fulfillment.sync_fulfillment",
	"orders/partially_fulfilled": "fateh_shopify_connector.fateh_shopify_connector.fulfillment.sync_fulfillment",
}
...
def get_access_token(store: "Document") -> str:
	...
	Token is stored by the oauth.callback() endpoint after successful authorization.
	...
	Raises:
		frappe.ValidationError: If token is missing (OAuth not yet authorized)
	"""
...
def get_callback_url(store: Document | None = None) -> str:
	...
	return f"https://{url}/api/method/fateh_shopify_connector.fateh_shopify_connector.connection.store_request_data"
...
def store_request_data() -> None:
	...
	if not store.enabled:
		logger.info("Shopify Store %s is disabled, skipping webhook", store.name)
		return

	# Validate HMAC - use shared_secret if set (manual override), else client_secret
	hmac_header = frappe.get_request_header("X-Shopify-Hmac-Sha256")
	webhook_secret = store.shared_secret or store.get_password("client_secret")
